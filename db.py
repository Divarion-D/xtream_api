...
class DB:
    ...
    def add_tables(self):
        ...
        self.cursor.execute('''CREATE TABLE IF NOT EXISTS settings ("setting_id" INTEGER PRIMARY KEY AUTOINCREMENT, "setting_name" TEXT, "setting_value" TEXT)''')
        # create table iptv_categories
        self.cursor.execute('''CREATE TABLE IF NOT EXISTS iptv_categories ("category_id" INTEGER PRIMARY KEY AUTOINCREMENT, "category_name" TEXT, "parent_id" INTEGER)''')
        ...
        self.cursor.execute('''CREATE TABLE IF NOT EXISTS iptv_epg ("epg_id" INTEGER PRIMARY KEY AUTOINCREMENT, "channel_id" INTEGER)''')
    ...
    def get_setting(self, setting_name):
        ...
        if self.get("settings", "setting_name", "setting_name = '{0}'".format(setting_name)) == None:
            ...
            self.insert("settings", "(setting_name, setting_value)", "('{0}', '')".format(setting_name))
    ...
    def set_setting(self, setting_name, setting_value):
        ...
        if self.get("settings", "setting_name", "setting_name = '{0}'".format(setting_name)) == None:
            # setting does not exist, create it
            self.insert("settings", "(setting_name, setting_value)", "('{0}', '{1}')".format(setting_name, setting_value))
        else:
            # setting exists, update it
            self.cursor.execute("UPDATE settings SET setting_value = '{0}' WHERE setting_name = '{1}'".format(setting_value, setting_name))
            self.db.commit()
