...
class M3U_Parser():
    ...
    def __init__(self, m3u_url):
        ...
        if channels_lupd < int(
                time.time()) - common.SETTING["iptv"]["upd_interval_list"]:
            # clear channels
        ...
        for line in self.m3u_list:
            if line.startswith('#EXTINF:'):
                ...
                data_list["stream_icon"] = re.search(
                    'tvg-logo="(.+?)"',
                    line).group(1).strip() if 'tvg-logo' in line else ""
                data_list["name"] = re.search('tvg-name="(.+?)"', line).group(
                    1).strip() if 'tvg-name' in line else re.search(
                        ',(.+)', line).group(1).strip()
                data_list["group_title"] = re.search(
                    'group-title="(.+?)"',
                    line).group(1).strip() if 'group-title' in line else ""
                data_list["epg_channel_id"] = re.search(
                    'tvg-id="(.+?)"',
                    line).group(1).strip() if 'tvg-id' in line else ""
                ...
                data_list["group_title"] = re.search('#EXTGRP:(.+)',
                                                     line).group(1)
            ...
            if cnl == 0 and data_list:
                ...
                if "group_title" in data_list:
                    ...
                    if not qb.select('iptv_categories').where([[
                            'category_name', '=', data_list["group_title"]
                    ]]).one():
                        qb.insert(
                            'iptv_categories', {
                                'category_name': data_list["group_title"],
                ...
                if not qb.select('iptv_channels').where(
                    [['name', '=', data_list["name"]]]).one():
                    # get category id
                    category_id = qb.select('iptv_categories').where(
                        [['category_name', '=',
                          data_list["group_title"]]]).one()['category_id']
                    # insert channel
                    insert_id = qb.insert(
                        'iptv_channels', {
                            'name': data_list["name"],
                    ...
                    # update stream_id
                    qb.update('iptv_channels', {
                        'stream_id': insert_id - 1
                    }).where([['channel_id', '=', insert_id]]).go()
                data_list = {}
    ...
    def get_channel_url(self, stream_id):
        channel = qb.select('iptv_channels').where(
