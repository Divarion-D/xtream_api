...
class QueryBuilder:
    ...
    def pluck(self,
              key: int = 0,
              column: int = 1) -> Union[tuple, list, dict, None]:
        self.query()
    ...
    def _prepare_aliases(self,
                         ...
                         as_list: bool = False) -> Union[str, list]:
            ...
            for item in items:
                if isinstance(items, list):
                    if isinstance(item, str):
                        ...
                        sql.append(f"{first_item}" if isinstance(alias, int)
                                   else f"{first_item} AS {alias}")
                elif isinstance(items, dict):
                    new_item = items[item]
                    sql.append(f"{new_item}" if isinstance(item, int) else
                               f"{new_item} AS {item}")
        else:
            self.set_error(
                f"Incorrect type of items in {inspect.stack()[0][3]} method")
            return ''
    ...
    def _prepare_conditions(self, where: Union[str, list]) -> dict:
        ...
        if isinstance(where, str):
                ...
                if isinstance(cond, list):
                    if len(cond) == 2:
                        ...
                        if operator in self._OPERATORS:
                            if operator == 'IN' and (isinstance(value, list) or
            ...
            self.set_error(
                f"Incorrect type of where in {inspect.stack()[0][3]} method")
            return result
    ...
    def select(self,
               ...
               fields: Union[str, list, dict] = '*'):
        if table == '' or table == {} or fields == '' or fields == [] or fields == {}:
            self.set_error(
                f"Empty table or fields in {inspect.stack()[0][3]} method")
            return self

        self.reset()

        if isinstance(fields, dict) or isinstance(fields, list) or isinstance(
                fields, str):
    ...
    def where(self, where: Union[str, list], addition: str = ''):
        ...
        if isinstance(conditions['values'],
                      list) and conditions['values'] != []:
    ...
    def insert(self, table: Union[str, dict], fields: Union[list, dict]):
        ...
        if isinstance(fields, dict):
            ...
            self._sql = f"INSERT INTO {table} (" + self._prepare_fieldlist(
                list(fields.keys())) + f") VALUES ({values})"
